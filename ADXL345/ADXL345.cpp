...
//  Wire.endTransmission();         // end transmission
  ...
  i2c.write(DEVICE, address, 1, &val);
}
...
//  }
  ...
  uint8_t i = i2c.read(DEVICE, address, num, _buff);
...
// if set to 1 it applies a self-test force to the sensor 
...
// if set to 1, the device is in full resolution mode, 
// where the output resolution increases with the
//   g range set by the range bits to maintain a 4mg/LSB scal factor
// if set to 0, the device is in 10-bit mode, and the range bits
...
void ADXL345::printByte(uint8_t val){
  int i;
  Serial.print("B");
  for(i=7; i>=0; i--){
